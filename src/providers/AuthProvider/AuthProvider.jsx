...
const AuthProvider = ({ children }) => {
  ...
  const createUser = async (name, email, password, membership) => {
    ...
    try {
      ...
      const userExistsResponse = await fetch(
        `${import.meta.env.VITE_API_Link(remove_it)}/users?email=${email}`
      );
      ...
      const backendResponse = await fetch(
        `${import.meta.env.VITE_API_Link(remove_it)}/users`,
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        fetch(`${import.meta.env.VITE_API_Link(remove_it)}/users?email=${currentUser.email}`)
          .then(async (res) => {
            if (!res.ok) {
  ...
  const fetchAllUsersData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_Link(remove_it)}/all-users`
      );
      if (!response.ok) {
        throw new Error(
  ...
  const updateUser = async (email, isAdmin) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_Link(remove_it)}/update-user`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const googleLogin = async () => {
    ...
    try {
      ...
      const userExistsResponse = await fetch(
        `${import.meta.env.VITE_API_Link(remove_it)}/users?email=${user.email}`
      );
  ...
  const fetchResortData = async () => {
    ...
    try {
      ...
      if (!import.meta.env.VITE_API_Link(remove_it)) {
        throw new Error("API link is not defined in environment variables.");
      }

      // Fetch resort data
      const response = await fetch(`${import.meta.env.VITE_API_Link(remove_it)}/allResorts`);
  ...
  const fetchAllResorts = async () => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_API_Link(remove_it)}/allResorts`;
...
const fetchBookingsData = async (email) => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_Link(remove_it)}/bookings?email=${email}`
    );
    if (!response.ok) {
      throw new Error(
...
const fetchAllBookingsData = async () => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_Link(remove_it)}/all-bookings`
    );
    if (!response.ok) {
      throw new Error(
